...
def _sanitize_region(region: NDArray, staff: Staff, image_shape: tuple[int, ...]) -> NDArray:
    x_min, y_min, x_max, y_max = region.astype(float)
    fallback = np.array(
        [
            math.floor(staff.min_x - 2 * staff.average_unit_size),
            math.floor(staff.min_y - 4 * staff.average_unit_size),
            math.ceil(staff.max_x + 2 * staff.average_unit_size),
            math.ceil(staff.max_y + 4 * staff.average_unit_size),
        ],
        dtype=float,
    )
    if x_max <= x_min or y_max <= y_min:
        eprint(
            "Warning: invalid staff crop",
            region.tolist(),
            "for staff",
            (staff.min_x, staff.min_y, staff.max_x, staff.max_y),
            "- falling back to padded staff bounds",
        )
...
def prepare_staff_image(
    ...
    region = _sanitize_region(region, staff, staff_image.shape)

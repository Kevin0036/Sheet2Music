...
def build_measures(
    ...
    pending_staff_events: dict[int, list[TimedNoteEvent]] = defaultdict(list)
    pending_segment_duration = 0

    def flush_pending_staff_events() -> None:
        nonlocal pending_segment_duration
        if not pending_staff_events:
            return

        staff_numbers = sorted(staff for staff, events in pending_staff_events.items() if events)
        if not staff_numbers:
            pending_staff_events.clear()
            pending_segment_duration = 0
            return

        for staff_index, staff_num in enumerate(staff_numbers):
            if staff_index > 0 and pending_segment_duration > 0:
                current_measure.append(build_backup_from_ticks(pending_segment_duration))
            for element in serialize_timed_events(pending_staff_events[staff_num]):
    ...
    current_time_in_measure = Fraction(0)
    for group_no, group in enumerate(groups):
        ...
        if rhythm.startswith(("note", "rest")):
            if len(group.symbols) == 1 and rhythm.endswith("m"):
                ...
                group_duration = group.get_duration()
                if has_two_staves:
                    start_ticks = int(current_time_in_measure * state.division)
                    group_ticks = int(group_duration * state.division)
                    for staff_pos in group.into_positions():
                        staff_num = get_staff(staff_pos.symbols[0])
                        pending_staff_events[staff_num].extend(
                            build_timed_note_events(staff_pos, state, start_ticks)
                        )
                    pending_segment_duration = start_ticks + group_ticks
                else:
                    staff_positions = group.into_positions()
                    for pos_no, staff_pos in enumerate(staff_positions):
                        chord_duration = (
                            group_duration if pos_no == len(staff_positions) - 1 else Fraction(0)
                        )
                        for note_xml in build_note_chord(staff_pos, state, chord_duration):
                            current_measure.append(note_xml)
                current_time_in_measure += group_duration
            continue
        flush_pending_staff_events()
        if rhythm == "newline":
    ...
    if len(list(current_measure)) > 0:
        close_current_measure()
    if not any(child.tag == "attributes" and child.find("time") is not None for child in measures[0]):
...
def build_backup_from_ticks(duration: int) -> ET.Element:
    assert duration > 0, "Backup duration must be positive"
    backup = ET.Element("backup")
    ET.SubElement(backup, "duration").text = str(duration)
    return backup


def build_forward_from_ticks(duration: int) -> ET.Element:
    assert duration > 0, "Forward duration must be positive"
...
def build_timed_note_events(
    note_chord: SymbolChord, state: ConversionState, start_ticks: int
) -> list[TimedNoteEvent]:
    by_duration = _group_notes(note_chord.symbols)
    events: list[TimedNoteEvent] = []
    staff_num = get_staff(note_chord.symbols[0])
    voice_index = 0
    for group_duration, group_notes in by_duration.items():
        duration_ticks = int(group_duration * state.division)
        notes = [n for n in group_notes if n.pitch not in (empty, nonote)]
        rests = [n for n in group_notes if n.pitch in (empty, nonote)]

        if notes:
            elements = [
                build_note_or_rest(note, voice_index, idx > 0, state, note_chord.tuplet_mark)
            ...
            events.append(
                TimedNoteEvent(
                    staff_num=staff_num,
                    start=start_ticks,
                    end=start_ticks + duration_ticks,
                    notes=elements,
                )
            )
            voice_index += 1
        ...
        if rests:
            events.append(
                TimedNoteEvent(
                    ...
                    notes=[
                        build_note_or_rest(
                            rests[0], voice_index, False, state, note_chord.tuplet_mark
                        )
                    ],
                )
            )
...
def serialize_timed_events(events: list[TimedNoteEvent]) -> list[ET.Element]:
    result: list[ET.Element] = []
    current_time = 0
    for event in sorted(events, key=lambda timed: (timed.start, timed.end)):
        if event.start < current_time:
            result.append(build_backup_from_ticks(current_time - event.start))
        elif event.start > current_time:
            result.append(build_forward_from_ticks(event.start - current_time))
        result.extend(event.notes)

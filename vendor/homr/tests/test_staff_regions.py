...
class TestStaffRegions(unittest.TestCase):
    ...
    def test_overlapping_fragment_is_not_treated_as_staff_below(self) -> None:
        system = MultiStaff(
            [
                build_staff(640, 760),
                build_staff(780, 900),
        ...
        next_system = MultiStaff([build_staff(1020, 1140)], [])
        regions = StaffRegions([system, next_system])

        self.assertEqual(760, regions.get_start_of_closest_staff_above(810))
        self.assertEqual(1020, regions.get_start_of_closest_staff_below(845))
